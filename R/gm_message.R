...
gm_modify_message <- function(
  ...
  gmailr_POST(
    c("messages", id, "modify"),
    user_id,
    class = "gmail_message",
    body = rename(
      "add_labels" = as.list(add_labels),
      "remove_labels" = as.list(remove_labels)
    ),
    encode = "json"
  )
}
...
#' }
gm_attachment <- function(id, message_id, user_id = "me") {
  stopifnot(
    is_string(id),
  ...
  gmailr_GET(
    c("messages", message_id, "attachments", id),
    user_id,
...
gm_attachments.gmail_message <- function(x, ...) {
  ...
  filename <- vcapply(x$payload$parts[has_attachments], function(part) {
    part$filename
  })
  type <- vcapply(x$payload$parts[has_attachments], function(part) {
    part$mimeType
  })
  size <- vnapply(x$payload$parts[has_attachments], function(part) {
    as.numeric(part$body$size)
  })
  id <- vcapply(x$payload$parts[has_attachments], function(part) {
...
gm_insert_message <- function(
  ...
  gmailr_POST(
    ...
    body = jsonlite::toJSON(
      auto_unbox = TRUE,
      c(not_null(rename(label_ids)), raw = base64url_encode(mail))
    ),
