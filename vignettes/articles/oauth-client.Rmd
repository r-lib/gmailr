...
That is only relevant to a client that aims to support external users.)
...
* Desktop app: use this if you're going to use gmailr on your local computer.
  ...
  This will also work in a hosted/cloud context, under very specific
  circumstances. The GCP project's OAuth consent screen must be configured for
  internal users within a Google workspace or for external users in "Testing"
  mode. This is associated with a conventional or legacy "out-of-bound (oob)
  auth flow.
* Web application: use this if you're going to use gmailr in some sort of hosted
  ...
  Google Colaboratory. This is the client type needed for a gargle-specific
  variant of oob known as pseudo-oob.
  
Note that above, we are talking about the computing environment where you will do the OAuth dance, i.e. where you will obtain a new token.
If you are going to do auth on your local computer, then deploy that token to Posit Connect or Cloud, you need a "Desktop app".
...
That has the potential downside that refresh tokens only last for 1 week (at the time of writing), i.e. the credential can be refreshed automatically for only 1 week.
...
1. The OAuth consent screen for the GCP project that owns the OAuth client may
   have publishing status "In production" (as opposed to "Testing"). This is
   associated with a gargle-specific variant of the "out-of-bound" (oob) auth
   flow known as pseudo-oob.
