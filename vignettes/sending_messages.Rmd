...
text_msg <- mime() %>%
  ...
  text_body("Gmailr is a very handy package!")
```
...
html_msg <- mime() %>%
  gm_to("james.f.hester@gmail.com") %>%
  gm_from("me@somewhere.com") %>%
  html_body("<b>Gmailr</b> is a <i>very</i> handy package!")
```
...
msg <- html_msg %>%
  attach_part(part = charToRaw("attach me!"), name = "please")
```
...
email <- gm_mime() %>%
  ...
  attach_file("mtcars.png", id = "foobar")
```
